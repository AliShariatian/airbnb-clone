...
const RentModal = () => {
   ...
   // for image upload step
   const imageSrc = watch("imageSrc");

   // dynamic import for map, when location step into view, map showing
   const Map = useMemo(
      () =>
         dynamic(() => import("../Map"), {
            ssr: false,
         ...
         <div className="grid grid-cols-1 md:grid-cols-2 gap-3 max-h-[50vh] overflow-y-auto">
            {categories.map((item) => (
               <div key={item.label} className="col-span-1">
                  <CategoryInput onClick={(value) => setCustomValue("category", value)} selected={category === item.label} label={item.label} icon={item.icon} />
               </div>
            ))}
         </div>
      </div>
   ...
   if (step === STEPS.LOCATION) {
      bodyContent = (
         <div className="flex flex-col gap-8">
            <Heading title="Where is your place is located?" subTitle="Help guests find you!" />
